...
- [x] Anmeldung mit E-Mail und Passwort steht — Registrierung, Anmeldung, Passwort zuruecksetzen, Kontoloeschung
- [ ] *(zurueckgestellt)* Google- und Apple-Anmeldung konfiguriert — [AUTH-PROVIDER.md](AUTH-PROVIDER.md). **Kein offener Rest, sondern eine Entscheidung:** V1 meldet ueber E-Mail und Passwort an. Beide Anbieter sind unabhaengig voneinander und blockieren nichts; ohne Einrichtung erscheint ihr Knopf gar nicht erst. Einschalten geht spaeter allein ueber das Supabase-Dashboard, **ohne Deployment**
- [ ] *(erst mit Apple)* Apple-Secret laeuft nach hoechstens 6 Monaten ab — Erinnerung eingetragen. Apple wird ausserdem erst Pflicht, wenn eine native App im Store eine andere Fremdanmeldung anbietet (Richtlinie 4.8) — native Apps sind nicht in V1 (CLAUDE.md)
