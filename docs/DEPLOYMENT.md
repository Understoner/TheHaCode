...
- [x] Beide Supabase-Projekte angelegt — **Migrationen** laufen ab jetzt über die Pipeline (`supabase db push`), `supabase/seed.sql` **nicht**: redaktionelle Inhalte kommen über das Studio (CLAUDE.md §2.4). Wer die vorkonfigurierten Sequenzen ändert, führt die Datei dort einmal je Umgebung im SQL Editor aus. Sie ersetzt den redaktionellen Bestand und lässt selbst gebaute Nutzersequenzen stehen.
